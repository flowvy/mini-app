...
- App shell и fullscreen editor остаются в layout viewport и не переписывают pixel geometry из
  ...
  BackButton. Основной Users route открывает настоящий search input только после перехода в эту
  task surface. Fullscreen-editor actions в поддерживаемом Telegram client передаются одним нативным
