...
Последняя полная проверка: **2026-08-24**; последний change-aware gate: **2026-08-24**. Все
не-accessibility стадии прошли; свежий mobile accessibility gate красный только на 13 тестах с
точными `color-contrast` nodes из принятого ledger ADR 0004, без новых rule/node pairs.
...
- Общие color/effect values и semantic color roles Mini App являются runtime-копией актуального
  ...
  glass family как явное исключение. Strict desktop palette создаёт известные `color-contrast` findings: Axe не
  фильтрует их, соответствующие scans остаются красными, а debt считается открытым. Решение:
  `docs/decisions/0004-desktop-color-parity.md`.
...
Для Support остаётся live Telegram acceptance: controlled request/user reply/admin reply должны
подтвердить реальные recipients и exact-route кнопки. Это отправит сообщения test bot и требует
