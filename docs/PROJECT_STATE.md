...
- Общие color/effect values и semantic color roles Mini App являются runtime-копией актуального
  ...
  `prefers-color-scheme` продолжают выбирать тему, Header сохраняет ограниченную glass family как
  явное исключение. Strict desktop palette создаёт известные `color-contrast` findings: Axe не
