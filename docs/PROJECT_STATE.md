...
- Payments содержит два независимых provider-wide referral benefits: один раз добавить
  ...
  скидочный CTA. Tribute подтверждает финальную сумму, поскольку применяет minimum EUR 1 и может
...
| Referral days and Tribute welcome discount | official Tribute promo-code and Telegram Mini Apps contracts checked 2026-08-22; focused backend/browser suites; migration verifier; Changed/Full gates | Payments получил два независимых toggles: fixed inviter days после первой applied Tribute grant и общий welcome promo link для приглашённого first-time payer. `referral_conversions` unique по invitee, поэтому replay/renewal/repeated donation и повторяемый donation ID не создают вторую награду; reward проходит через existing ledger/executor с frozen automation profile. Backend выбирает promo до immutable checkout snapshot только для configured published subscription. Admin задаёт 1–99%, а Home продаёт benefit отдельным блоком, зачёркивает provider prices, показывает рассчитанные first-payment prices для всех billing periods и предупреждает, что Tribute подтверждает финальную сумму. Focused backend 73/73 и browser 8/8 прошли; light/dark evidence на 320/430, WebKit и desktop просмотрены. Fresh Full gate прошёл migration lifecycle/drift, Ruff, 533 backend, 56 pinned contracts, frontend lint/typecheck/78 unit/build, 150/150 mobile Playwright и docs. Реальные provider/payment mutations, commit и push не выполнялись. |
## Следующее действие
