		...
		>
			<div className={styles.header}>
				<div>
					...
					<h2 id="access-profile-editor-title">
				...
				{draft.validityMode === "duration" && (
					...
					>
						<FormFieldInput
							...
							onChange={(event) =>
								setDraft({ ...draft, validityDays: Number(event.target.value) || null })
							}
						/>
					</FormField>
				)}
					...
					<FormInlineField label={t("access.expireAt")} htmlFor="access-profile-expiry">
						<FormInlineDate
							...
							onChange={(event) =>
								setDraft({
									...draft,
									fixedExpireAt: event.target.value ? endOfLocalDay(event.target.value) : null,
								})
							}
						/>
					</FormInlineField>
